...
def load_vix_with_features():
    ...
    return df[["date", "vix_level", "vix_direction"]]
...
def main():
    print("Running Phase-1.75 backtest (Time + VIX gated)...")
    ...
    trades.to_csv(
        os.path.join(OUTPUT_DIR, "scenario3_phase1_75_trades.csv"),
        index=False
    )

    pd.DataFrame([summary]).to_csv(
        os.path.join(OUTPUT_DIR, "scenario3_phase1_75_summary.csv"),
        index=False
    )

    print("Phase-1.75 backtest completed")
    print(summary)
